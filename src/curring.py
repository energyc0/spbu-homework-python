...
def curry(func, argc):
    ...
    arg_count = sum(1 for arg in sig.parameters.values()
...
def uncurry(func, argc):
    '''
    This function turns curried function into normal function with given 'argc'
    arguments count. User must give correct number of arguments,
    otherwise function will return the curried function with reduced number of arguments.
    ...
    def _uncurried(*args):
        if len(args) != argc:
            raise TypeError(f"Function takes {argc} arguments, but {len(args)} were given.")
